...
int main() {
    struct stat st;
    const char *nazwa_pliku = "test.txt"; // Podaj istniejący plik

    if (stat(nazwa_pliku, &st) == 0) {
        // st.st_size przechowuje rozmiar pliku w bajtach
        printf("Wielkość pliku '%s' wynosi: %ld bajtów\n", nazwa_pliku, st.st_size);
    } else {
        perror("Błąd podczas używania stat (plik nie istnieje?)");
    }
    return 0;
}
*/